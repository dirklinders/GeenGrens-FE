...
export function CharacterSelector({ characters, selectedId, lockedIds, onSelect }: CharacterSelectorProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      {characters.map((character) => {
        const isLocked = lockedIds?.has(character.id) ?? false;
          ...
          >
            ...
            <div
              className={cn(
                ...
                isSelected ? 'bg-red-800 text-red-100' : 'bg-stone-700 text-stone-300'
              )}
            >
              {character.avatarUrl ? (
                <img
            ...
            <div className="flex-1 min-w-0">
              ...
              {isLocked ? (
                <div className="text-xs text-stone-600 truncate">🔒 Verhoor afgesloten</div>
              ) : character.description ? (
                <div className="text-xs text-stone-500 truncate">{character.description}</div>
              ) : null}
            </div>
          </button>
        );
